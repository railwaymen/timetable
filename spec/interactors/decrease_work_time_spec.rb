...
describe DecreaseWorkTime do
  ...
  it 'does nothing when there is no matching period' do
    expect { decrease_work_time(2.hours, Time.zone.parse('2015-09-01')) }.not_to raise_error
  end

  context 'full-time' do
    it 'decreases time from matching period' do
      params = { full_time: true, starts_at: Time.zone.parse('2015-09-01'), ends_at: Time.zone.parse('2015-09-30'), counted_duration: 168.hours, duration: 168.hours, user: user, position: 1 }
      period = create :accounting_period, params

      decrease_work_time(2.hours, Time.zone.parse('2015-09-01'))
  ...
  context 'contractor' do
    ...
    it 'splits time accross next periods' do
      ...
      decrease_work_time(8.hours, Time.zone.parse('2015-09-01'))
