...
export default function ArchivedContent({ phrase, visible }) {
  ...
  useEffect(() => {
    makeGetRequest({
      url: `/api/hardware_devices/archived?page=${1}&q=${phrase}`,
  ...
  useEffect(() => {
    makeGetRequest({
      url: `/api/hardware_devices/archived?page=${page}&q=${phrase}`,
    }).then(({ data: { data, total_pages } }) => {
      setContent({
