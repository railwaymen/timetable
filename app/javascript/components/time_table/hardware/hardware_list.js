...
const HardwareList = () => {
  ...
  function getComapnies() {
    Api.makeGetRequest({ url: '/api/companies' })
      .then((response) => {
        setCompanies(response.data);
      });
  }

  function onAgreementButtonClick(type) {
    if (companies.length === 0) { getComapnies(); }
    ...
    return (
      <div className="btn-group bg-white ml-5">
        <ModalButton
          ...
          onClick={() => onAgreementButtonClick('rental')}
        ...
        <ModalButton
          ...
          onClick={() => onAgreementButtonClick('return')}
  ...
  function DeleteModal() {
    const count = currentUser.isHardwareManager() ? selectedHardwares.length : selectedHardwares.filter((sh) => sh.status === 'in_office').length;
    return (
      <div>
        { selectedStatuses.length !== 1 && <DeleteModalWarning /> }
        {I18n.t('apps.hardware.confirm_delete', { count })}
      </div>
    );
  }
      ...
      <form className="form ui">
        ...
        <div className="fields inline">
          <div className="field">
            ...
            <select
              onChange={(e) => setSelectedCompany(companies.find((c) => c.id === parseInt(e.target.value, 10)))}
