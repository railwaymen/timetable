...
module Api
  class HardwaresController < Api::BaseController
    ...
    def index
      hardwares = Hardware.includes(:user, :hardware_fields).all
    ...
    def rental_agreement
      ...
      # send_data generator.generate, filename: 'rental_agreement.pdf'
    end
    ...
    def return_agreement
      ...
      # send_data generator.generate, filename: 'return_agreement.pdf'
    end

    def change_status
      @hardware = policy_scope(Hardware.where(id: params[:hardware_id])).first
      Rails.logger.info permitted_attributes(@hardware)
